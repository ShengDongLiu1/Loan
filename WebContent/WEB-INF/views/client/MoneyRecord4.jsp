	...
	<table class="table" style="border: 2px solid; border-color: #DFF0D8;">
				<thead>
					<tr>
						...
						<th width="150px">冻结状态</th>
						<th width="150px">冻结金额</th>
						<th width="150px">冻结备注</th>
					
					</tr>
				</thead>
				<tbody>
							<c:forEach var="list" items="${frolist.rows}" >
								<tr class="success">
									<td><fmt:formatDate value="${list.ftime}" type="both"/></td>
									<td>
										<c:choose>
											<c:when test="${list.fstate == 1}">已冻结</c:when>
											<c:when test="${list.fstate == 2}">未冻结</c:when>
			...
			<div>
								<a href="<%=path %>/capital/selectAngodiv1?state=4&page=${frolist.pageNo - 1}"  style="background:#224762;color:white;font-size:15px;border-radius:5px;text-decoration:none;">上一页</a>&nbsp;&nbsp;
								<a href="<%=path %>/capital/selectAngodiv1?state=4&page=${frolist.pageNo + 1}" style="background:#224762;color:white;font-size:15px;border-radius:5px;text-decoration:none;">下一页</a>&nbsp;&nbsp;
								当前页 第<span id="page">${frolist.pageNo}</span>页&nbsp;&nbsp;
								共<span id="count">${frolist.total}</span>页&nbsp;&nbsp;
